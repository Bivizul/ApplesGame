...
namespace ApplesGame
{
	void InitGameStatePlaying(GameStatePlayingData& data, Game& game)
	{
		...
		// Init rocks
		data.rocks.clear();
		int numRocks = MIN_ROCKS + rand() % (MAX_ROCKS + 1 - MIN_ROCKS);
		data.rocks.resize(numRocks);
		for (Rock& rock : data.rocks)
		{
			InitRocks(rock, data.rockTexture);
		}
	...
	void UpdateGameStatePlaying(GameStatePlayingData& data, Game& game, float timeDelta)
	{
		...
		for (Rock& rock : data.rocks)
		{
			if (FindPlayerCollisionWithRock(data.player.position, rock.position))
			{
